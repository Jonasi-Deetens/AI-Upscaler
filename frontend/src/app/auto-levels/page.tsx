...
const MODE_OPTIONS: { value: "levels" | "contrast"; label: string }[] = [
  { value: "levels", label: "Auto levels (stretch histogram)" },
  { value: "contrast", label: "Auto contrast (equalize)" },
];

export default function AutoLevelsPage() {
  const router = useRouter();
  const [files, setFiles] = useState<File[]>([]);
  const [mode, setMode] = useState<"levels" | "contrast">("levels");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { job_ids } = await uploadJobsWithProgress(
        files,
        { scale: 1, method: "auto_levels", options },
    ...
    <main className="min-h-screen bg-background text-foreground">
      <div className="max-w-6xl mx-auto px-5 py-12">
        ...
        <p className="text-muted-foreground mb-6">
          One-click histogram adjustment. Levels stretches the range; contrast equalizes the histogram for stronger normalization.
        </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="rounded-2xl bg-card border border-border p-5 shadow-sm space-y-4">
            <div>
              ...
              <select
                value={mode}
                onChange={(e) => setMode(e.target.value as "levels" | "contrast")}

...
export default function BackgroundReplacePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (files.length !== 2) {
      setError("Upload exactly 2 files: subject image first, then background image.");
      return;
    }
    setError(null);
    ...
    try {
      const { job_ids } = await uploadJobsWithProgress(
        [files[0], files[1]],
        { scale: 1, method: "background_replace" },
    ...
    <main className="min-h-screen bg-background text-foreground">
      <div className="max-w-6xl mx-auto px-5 py-12">
        ...
        <p className="text-muted-foreground mb-6">
          Put the subject from the first image onto the background of the second. Upload subject first, then background. Order matters.
        </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <p className="text-sm text-muted-foreground">
            Select exactly 2 files: 1) subject (person/object), 2) new background image.
          </p>
          {uploadProgress != null && (

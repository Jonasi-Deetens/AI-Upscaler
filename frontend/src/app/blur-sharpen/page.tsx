...
export default function BlurSharpenPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const options: Record<string, unknown> = {
        mode,
        radius: mode === "blur" ? radius : 3,
        strength: mode === "sharpen" ? strength : 1.5,
      };
      const { job_ids } = await uploadJobsWithProgress(
        files,
        { scale: 1, method: "blur_sharpen", options },
    ...
    <main className="min-h-screen bg-background text-foreground">
      <div className="max-w-6xl mx-auto px-5 py-12">
        ...
        <p className="text-muted-foreground mb-6">
          Apply Gaussian blur or unsharp mask to images. Choose mode and adjust strength.
        </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="rounded-2xl bg-card border border-border p-5 shadow-sm space-y-4">
            <div>
              ...
              <select
                value={mode}
                onChange={(e) => setMode(e.target.value as "blur" | "sharpen")}
            ...
            {mode === "blur" && (
              <div>
                ...
                <input
                  ...
                  className="w-full"
                />
                <span className="ml-2 text-sm text-muted-foreground">{radius}</span>
              </div>
            )}
            {mode === "sharpen" && (
              <div>
                <label className="block text-sm font-medium text-foreground mb-2">
                  Sharpen strength (0.5–5)
                </label>
                <input
                  type="range"
                  min={0.5}
                  ...
                  className="w-full"
                />
                <span className="ml-2 text-sm text-muted-foreground">{strength.toFixed(1)}</span>
              </div>
            )}
          </div>
          {uploadProgress != null && (

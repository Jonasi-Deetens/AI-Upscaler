...
export default function BorderPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setUploadProgress(0);
    try {
      const options = { padding, color, width };
      const { job_ids } = await uploadJobsWithProgress(
        files,
        { scale: 1, method: "border", options },
    ...
    <main className="min-h-screen bg-background text-foreground">
      <div className="max-w-6xl mx-auto px-5 py-12">
        ...
        <p className="text-muted-foreground mb-6">
          Add padding and optional frame around the image.
        </p>
          ...
          <div className="rounded-2xl bg-card border border-border p-5 shadow-sm space-y-4">
            <div>
              ...
              <input
                ...
                max={500}
                value={padding}
                onChange={(e) => setPadding(Math.min(500, Math.max(0, parseInt(e.target.value, 10) || 0)))}
            ...
            <div>
              ...
              <input
                ...
                max={100}
                value={width}
                onChange={(e) => setWidth(Math.min(100, Math.max(0, parseInt(e.target.value, 10) || 0)))}
                className="w-full rounded-lg border border-input bg-background px-3 py-2 text-foreground"
              />
            </div>
          </div>

  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { job_ids } = await uploadJobsWithProgress(
        files,
        { scale: 1, method: "brightness_contrast", options },
    ...
    <main className="min-h-screen bg-background text-foreground">
      <div className="max-w-6xl mx-auto px-5 py-12">
        ...
        <p className="text-muted-foreground mb-6">
          Adjust brightness (-100 to 100, 0 = no change) and contrast (0 to 200, 100 = no change).
        </p>
            ...
            <div>
              ...
              <div className="flex items-center gap-3">
                ...
                <input
                  ...
                  max={100}
                  value={brightness}
                  onChange={(e) => setBrightness(Math.min(100, Math.max(-100, parseInt(e.target.value, 10) || 0)))}
                  className="w-16 rounded-lg border border-input bg-background px-2 py-1 text-foreground text-sm"
                />
              </div>
            </div>
            <div>
              ...
              <div className="flex items-center gap-3">
                ...
                <input
                  ...
                  max={200}
                  value={contrast}
                  onChange={(e) => setContrast(Math.min(200, Math.max(0, parseInt(e.target.value, 10) || 100)))}
                  className="w-16 rounded-lg border border-input bg-background px-2 py-1 text-foreground text-sm"
                />
              </div>
            </div>

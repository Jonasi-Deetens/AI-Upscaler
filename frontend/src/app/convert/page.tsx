...
import type { ConvertTargetFormat } from "@/lib/types";

const FORMAT_OPTIONS: { value: ConvertTargetFormat; label: string }[] = [
...
export default function ConvertPage() {
  ...
  const [targetFormat, setTargetFormat] = useState<ConvertTargetFormat>("webp");
  ...
  const showQuality = targetFormat === "webp" || targetFormat === "jpeg";
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="rounded-2xl bg-white/70 dark:bg-zinc-800/60 backdrop-blur-sm border border-white/80 dark:border-zinc-700/80 p-5 shadow-sm">
            <RadioGroup
              ...
              value={targetFormat}
              onChange={(v) => setTargetFormat(v as ConvertTargetFormat)}
            />
          </div>
          {showQuality && (
            <div className="rounded-2xl bg-white/70 dark:bg-zinc-800/60 backdrop-blur-sm border border-white/80 dark:border-zinc-700/80 p-5 shadow-sm">

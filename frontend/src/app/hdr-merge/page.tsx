...
export default function HdrMergePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (files.length < 3) {
      setError("Upload at least 3 bracketed exposures (e.g. dark, mid, bright).");
      return;
    }
    setError(null);
    ...
    <main className="min-h-screen bg-background text-foreground">
      <div className="max-w-6xl mx-auto px-5 py-12">
        ...
        <p className="text-muted-foreground mb-6">
          Combine 3 or more bracketed exposures into one balanced image. Order: dark to bright (or any order).
        </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <p className="text-sm text-muted-foreground">
            Select at least 3 images (bracketed exposures of the same scene).
          </p>
          {uploadProgress != null && (

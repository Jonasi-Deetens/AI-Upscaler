...
export default function HeicToJpgPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (files.length === 0) {
      setError("Select at least one HEIC/HEIF file");
      return;
    }
    setError(null);
    ...
    try {
      const { job_ids } = await uploadJobsWithProgress(
        files,
        { scale: 1, method: "heic_to_jpg", options: {} },
    ...
    <main className="min-h-screen bg-background text-foreground">
      <div className="max-w-6xl mx-auto px-5 py-12">
        ...
        <p className="text-muted-foreground mb-6">
          Convert HEIC/HEIF images to JPEG. One job per file; download the JPG from Jobs.
        </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <FileDropzone
            ...
            accept=".heic,.heif,image/heic,image/heif-sequence"
            showPreview={false}
            maxFiles={10}
            className="mb-4"
          />

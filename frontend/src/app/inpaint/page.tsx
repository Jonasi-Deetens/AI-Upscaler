...
export default function InpaintPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (files.length !== 2) {
      setError("Upload exactly 2 files: image first, then mask (white = area to fill)");
      return;
    }
    setError(null);
  ...
  return (
    <main className="min-h-screen bg-background text-foreground">
      <div className="max-w-6xl mx-auto px-5 py-12">
        ...
        <h1 className="text-3xl font-bold text-foreground mb-6">Inpaint</h1>
        <p className="text-muted-foreground mb-6">
          Content-aware fill: upload the image and a mask. <strong>White (or bright) areas on the mask</strong> are the regions to fill; black areas are kept. Order: image first, mask second.
        </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <p className="text-sm text-muted-foreground">
            Select exactly 2 files: 1) image, 2) mask (grayscale or RGB; white = inpaint region).
          </p>
          {uploadProgress != null && (

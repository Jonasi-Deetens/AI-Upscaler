...
export default function ObjectRemovePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (files.length !== 2) {
      setError("Upload exactly 2 files: image first, then mask (white = area to remove)");
      return;
    }
    setError(null);
    ...
    <main className="min-h-screen bg-background text-foreground">
      <div className="max-w-6xl mx-auto px-5 py-12">
        ...
        <p className="text-muted-foreground mb-6">
          Remove objects or people using AI (LaMa). Upload the image and a mask: <strong>white areas on the mask</strong> are the regions to remove; black areas are kept. Order: image first, mask second.
        </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <p className="text-sm text-muted-foreground">
            Select exactly 2 files: 1) image, 2) mask (grayscale or RGB; white = area to remove).
          </p>
          {uploadProgress != null && (

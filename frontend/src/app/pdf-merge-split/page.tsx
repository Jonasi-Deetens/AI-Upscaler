...
export default function PdfMergeSplitPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (action === "split" && files.length !== 1) {
      setError("Split requires exactly one PDF file");
      return;
    }
    if (action === "merge" && files.length === 0) {
      setError("Select at least one PDF to merge");
      return;
    }
    setError(null);
    ...
    try {
      ...
      const { job_ids } = await uploadJobsWithProgress(
        files,
        { scale: 1, method: "pdf_merge_split", options },
    ...
    <main className="min-h-screen bg-background text-foreground">
      <div className="max-w-6xl mx-auto px-5 py-12">
        ...
        <p className="text-muted-foreground mb-6">
          Merge multiple PDFs into one, or split one PDF into separate pages (ZIP download).
        </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="rounded-2xl bg-card border border-border p-5 shadow-sm space-y-4">
            <div>
              ...
              >
                <option value="merge">Merge PDFs (multiple files → one PDF)</option>
                <option value="split">Split PDF (one file → ZIP of pages)</option>
              </select>
            </div>
          </div>
          <FileDropzone
            onFilesSelected={handleFilesSelected}
            maxFiles={action === "split" ? 1 : 50}
          ...
          <p className="text-sm text-muted-foreground">
            {action === "merge"
              ? "Upload one or more PDFs. They will be merged in order."
              : "Upload a single PDF. You will get a ZIP with one PDF per page."}
          </p>
          {uploadProgress != null && (

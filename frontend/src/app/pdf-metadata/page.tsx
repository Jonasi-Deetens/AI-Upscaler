  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { job_ids } = await uploadJobsWithProgress(
        files,
        { scale: 1, method: "pdf_metadata", options: { strip } },
    ...
    <main className="min-h-screen bg-background text-foreground">
      <div className="max-w-6xl mx-auto px-5 py-12">
        ...
        <p className="text-muted-foreground mb-6">
          Download a copy of the PDF with metadata preserved, or strip metadata (title, author, etc.) for privacy.
        </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex items-center gap-3">
            ...
            />
            <label htmlFor="strip" className="text-sm font-medium text-foreground">
              Strip metadata (output PDF without title, author, etc.)
            </label>
          </div>
          {uploadProgress != null && (
          ...
          >
            {loading
              ? uploadProgress != null
                ...
                : "Uploading…"
              : strip ? "Upload and strip metadata" : "Upload and download copy"}
          </Button>
        </form>
      </div>
    </main>

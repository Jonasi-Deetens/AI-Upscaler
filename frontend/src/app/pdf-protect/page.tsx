...
export default function PdfProtectPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!userPassword.trim()) {
      setError("Enter a user password (required to open the PDF)");
      return;
    }
    setError(null);
    ...
    setUploadProgress(0);
    try {
      const options: { user_password: string; owner_password?: string } = {
        user_password: userPassword.trim(),
      };
      if (ownerPassword.trim()) options.owner_password = ownerPassword.trim();
      const { job_ids } = await uploadJobsWithProgress(
        files,
        { scale: 1, method: "pdf_protect", options },
    ...
    <main className="min-h-screen bg-background text-foreground">
      <div className="max-w-6xl mx-auto px-5 py-12">
        ...
        <p className="text-muted-foreground mb-6">
          Add password protection to PDFs. User password is required to open the file; owner password is optional. One job per file.
        </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <input
              ...
              className="w-full max-w-md rounded-lg border border-border bg-background px-3 py-2 text-foreground focus:outline-none focus-visible:ring-2 focus-visible:ring-ring"
            />
          </div>
          <div className="space-y-2">
            <label htmlFor="ownerPassword" className="text-sm font-medium text-foreground">
          ...
          <Button
            ...
            variant="cta"
            size="lg"
            disabled={loading || files.length === 0 || !userPassword.trim()}

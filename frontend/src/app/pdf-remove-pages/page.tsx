...
function parsePagesToRemove(value: string): number[] | null {
...
export default function PdfRemovePagesPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const pagesToRemove = parsePagesToRemove(pagesInput);
    if (!pagesToRemove || pagesToRemove.length === 0) {
      setError("Enter page numbers to remove (e.g. 2, 5, 7). Comma-separated, 1-based.");
      return;
    }
    setError(null);
    ...
    try {
      const { job_ids } = await uploadJobsWithProgress(
        files,
        { scale: 1, method: "pdf_remove_pages", options: { pages_to_remove: pagesToRemove } },
    ...
    <main className="min-h-screen bg-background text-foreground">
      <div className="max-w-6xl mx-auto px-5 py-12">
        ...
        <p className="text-muted-foreground mb-6">
          Remove specific pages from PDFs. Enter 1-based page numbers to remove (e.g. 2, 5, 7). One job per file; same pages are removed from each file in the batch.
        </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-2">
            <label htmlFor="pages" className="text-sm font-medium text-foreground">
              Pages to remove (1-based, comma-separated)
            </label>
            <input
              id="pages"
              type="text"
              ...
              className="w-full max-w-md rounded-lg border border-border bg-background px-3 py-2 text-foreground placeholder:text-muted-foreground focus:outline-none focus-visible:ring-2 focus-visible:ring-ring"
            />
            <p className="text-xs text-muted-foreground">
              List the page numbers you want to remove. At least one page must remain in each PDF.
            </p>
          </div>
          {uploadProgress != null && (

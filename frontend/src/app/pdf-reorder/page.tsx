...
function parsePageOrder(value: string): number[] | null {
...
export default function PdfReorderPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const pageOrder = parsePageOrder(pageOrderInput);
    if (!pageOrder || pageOrder.length === 0) {
      setError("Enter page order as 1-based numbers, e.g. 3, 1, 2");
      return;
    }
    setError(null);
    ...
    try {
      const { job_ids } = await uploadJobsWithProgress(
        files,
        { scale: 1, method: "pdf_reorder", options: { page_order: pageOrder } },
    ...
    <main className="min-h-screen bg-background text-foreground">
      <div className="max-w-6xl mx-auto px-5 py-12">
        ...
        <p className="text-muted-foreground mb-6">
          Reorder pages by specifying the new order with 1-based page numbers. One job per PDF; use the same order for all files in the batch.
        </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-2">
            <label htmlFor="pageOrder" className="text-sm font-medium text-foreground">
              Page order (1-based, comma-separated)
            </label>
            <input
              id="pageOrder"
              type="text"
              ...
              className="w-full max-w-md rounded-lg border border-border bg-background px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus-visible:ring-2 focus-visible:ring-ring"
            />
            <p className="text-xs text-muted-foreground">
              List each page number once in the order you want. Example: 3, 1, 2 puts page 3 first, then 1, then 2.
            </p>
          </div>
          {uploadProgress != null && (

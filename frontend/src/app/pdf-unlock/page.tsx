  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { job_ids } = await uploadJobsWithProgress(
        files,
        { scale: 1, method: "pdf_unlock", options: { password: password.trim() } },
    ...
    <main className="min-h-screen bg-background text-foreground">
      <div className="max-w-6xl mx-auto px-5 py-12">
        ...
        <p className="text-muted-foreground mb-6">
          Remove password protection from PDFs. Enter the current password; one job per file. Download the unlocked PDF from Jobs.
        </p>

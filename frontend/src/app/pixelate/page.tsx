...
export default function PixelatePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setUploadProgress(0);
    try {
      const options = { block_size: blockSize };
      const { job_ids } = await uploadJobsWithProgress(
        files,
        { scale: 1, method: "pixelate", options },
  ...
  return (
    <main className="min-h-screen bg-background text-foreground">
      <div className="max-w-6xl mx-auto px-5 py-12">
        ...
        <h1 className="text-3xl font-bold text-foreground mb-6">Pixelate</h1>
        <p className="text-muted-foreground mb-6">
          Mosaic effect: downscale then upscale for a blocky look. Useful for privacy or stylized blur.
        </p>
          ...
          <div className="rounded-2xl bg-card border border-border p-5 shadow-sm space-y-4">
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setBlockSize(
                    Math.min(128, Math.max(2, parseInt(e.target.value, 10) || 2))
                  )
                }

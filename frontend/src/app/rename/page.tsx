...
export default function RenamePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setUploadProgress(0);
    try {
      const options = { prefix: prefix.trim() };
      const { job_ids } = await uploadJobsWithProgress(
        files,
        { scale: 1, method: "rename", options },
    ...
    <main className="min-h-screen bg-background text-foreground">
      <div className="max-w-6xl mx-auto px-5 py-12">
        ...
        <p className="text-muted-foreground mb-6">
          Add an optional prefix to output filenames. Results will be named <code className="text-sm bg-muted px-1 rounded">prefix_originalname.png</code> or <code className="text-sm bg-muted px-1 rounded">originalname_renamed.png</code> if no prefix.
        </p>
          ...
          <div className="rounded-2xl bg-card border border-border p-5 shadow-sm space-y-4">
            <div>
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                Only letters, numbers, underscore and hyphen are kept. Leave empty for &quot;originalname_renamed.png&quot;.
              </p>
            </div>
          </div>
          {uploadProgress != null && (

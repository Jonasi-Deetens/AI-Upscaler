...
export default function SaturationPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setUploadProgress(0);
    try {
      const options = { saturation, vibrance };
      const { job_ids } = await uploadJobsWithProgress(
        files,
        { scale: 1, method: "saturation", options },
    ...
    <main className="min-h-screen bg-background text-foreground">
      <div className="max-w-6xl mx-auto px-5 py-12">
        ...
        <p className="text-muted-foreground mb-6">
          Adjust saturation and vibrance (0–200, 100 = no change).
        </p>
            ...
            <div>
              ...
              <div className="flex items-center gap-3">
                <input
                  ...
                  className="flex-1"
                />
                <span className="text-sm text-muted-foreground w-10">{saturation}</span>
              </div>
            </div>
            <div>
              <label className="block text-sm font-medium text-foreground mb-2">
              ...
              <div className="flex items-center gap-3">
                <input
                  ...
                  className="flex-1"
                />
                <span className="text-sm text-muted-foreground w-10">{vibrance}</span>
              </div>
            </div>
          </div>
          {uploadProgress != null && (

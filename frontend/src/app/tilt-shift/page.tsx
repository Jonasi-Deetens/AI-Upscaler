...
export default function TiltShiftPage() {
  ...
  const [focusWidth, setFocusWidth] = useState(30);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const options = {
        ...
        focus_center: focusCenter / 100,
        focus_width: focusWidth / 100,
      };
      const { job_ids } = await uploadJobsWithProgress(
        files,
        { scale: 1, method: "tilt_shift", options },
  ...
  return (
    <main className="min-h-screen bg-background text-foreground">
      <div className="max-w-6xl mx-auto px-5 py-12">
        ...
        <h1 className="text-3xl font-bold text-foreground mb-6">Tilt-shift</h1>
        <p className="text-muted-foreground mb-6">
          Fake miniature effect: sharp band with blurred top and bottom.
        </p>
          ...
          <div className="rounded-2xl bg-card border border-border p-5 shadow-sm space-y-4">
            ...
            <div>
              ...
              <input
                ...
                max={100}
                value={focusCenter}
                onChange={(e) => setFocusCenter(parseInt(e.target.value, 10))}
                className="w-full"
              />
              <span className="text-sm text-muted-foreground ml-2">{focusCenter}%</span>
            </div>
            <div>
              <label className="block text-sm font-medium text-foreground mb-2">
                Focus width (% of image height, 5–100)
              </label>
              <input
                type="range"
                min={5}
                max={100}
                value={focusWidth}
                onChange={(e) => setFocusWidth(parseInt(e.target.value, 10))}
                className="w-full"
              />
              <span className="text-sm text-muted-foreground ml-2">{focusWidth}%</span>
            </div>
          </div>
          {uploadProgress != null && (

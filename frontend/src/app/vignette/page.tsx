...
export default function VignettePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setUploadProgress(0);
    try {
      const options = { strength, radius };
      const { job_ids } = await uploadJobsWithProgress(
        files,
        { scale: 1, method: "vignette", options },
  ...
  return (
    <main className="min-h-screen bg-background text-foreground">
      <div className="max-w-6xl mx-auto px-5 py-12">
        ...
        <h1 className="text-3xl font-bold text-foreground mb-6">Vignette</h1>
        <p className="text-muted-foreground mb-6">
          Darken the corners with a radial gradient for a classic look.
        </p>
          ...
          <div className="rounded-2xl bg-card border border-border p-5 shadow-sm space-y-4">
            <div>
              ...
              <input
                ...
                className="w-full"
              />
              <span className="text-sm text-muted-foreground ml-2">{strength}</span>
            </div>
            <div>
              <label className="block text-sm font-medium text-foreground mb-2">
                Radius (0–100, bright center size)
              </label>
              <input
                type="range"
                min={0}
                ...
                className="w-full"
              />
              <span className="text-sm text-muted-foreground ml-2">{radius}</span>
            </div>
          </div>
          {uploadProgress != null && (
